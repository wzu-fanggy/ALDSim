...
#ifdef APP_CLASS
AppStyle(ald/zns,AppAldZns)

#else

#ifndef SPK_app_ald_zns_H
#define SPK_app_ald_zns_H

#include "app_lattice.h"

namespace SPPARKS_NS {

class AppAldZns : public AppLattice {
  friend class DiagAldZns;

 public:
  AppAldZns(class SPPARKS *, int, char **);
  ~AppAldZns();
 ...
 private:
  ...
  void count_coordS(int);
  void count_coordZn(int);
  void remove_mask(int, int j = 0);
  void put_mask(int);
  void update_coord(int,int,int,int,int);
  void track_msite(int);

  // event-list / solver-propensity consistency self-healing (port from AppAld)
  void audit_consistency();
  void dump_crash_context(const char *, int, int, int, int, int);
  int heal_desync(int);
  void free_events_safe(int);
  int audit_counter;
  int heal_count;
};
